...
class SpriteRenderer
{
 public:
   ...
   [[nodiscard]] render_objects::Sprite create_sprite(triglav::ResourceName textureName);
   [[nodiscard]] render_objects::Sprite create_sprite_from_texture(const graphics_api::Texture& texture);

   void update_resolution(const graphics_api::Resolution& resolution);
   void begin_render(graphics_api::CommandList& cmdList) const;
   void draw_sprite(graphics_api::CommandList& cmdList, const render_objects::Sprite& sprite, glm::vec2 position, glm::vec2 scale) const;
