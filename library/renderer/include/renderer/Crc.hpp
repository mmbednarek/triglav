...
constexpr uint32_t compute_crc(const char *data, const uint32_t len, uint32_t crc = 0)
{
   crc = crc ^ 0xFFFFFFFFU;
...
#define CRC_HASH_STRING(value) std::integral_constant<uint32_t, compute_crc(value, sizeof(value) - 1)>::value
