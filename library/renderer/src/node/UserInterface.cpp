...
UserInterface::UserInterface(graphics_api::Device &device, const graphics_api::Resolution resolution,
    ...
    m_textureRenderTarget(GAPI_CHECK(graphics_api::RenderTargetBuilder(device).attachment("user_interface"_name_id,
...
void UserInterface::add_label_group(NameID id, const std::string_view name)
{
   LabelGroup labelGroup{
      .m_groupTitle = m_textRenderer.create_text_object(m_resourceManager.get<ResourceType::GlyphAtlas>("segoeui/bold/20.glyphs"_name), name)
   };
...
void UserInterface::add_label(NameID group, const NameID id, const std::string_view name)
{
   m_properties.emplace(
           id, TextProperty{m_textRenderer.create_text_object(
                                    m_resourceManager.get<ResourceType::GlyphAtlas>("segoeui/regular/18.glyphs"_name),
                                    name),
