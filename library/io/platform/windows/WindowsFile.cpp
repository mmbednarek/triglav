...
namespace triglav::io::windows {
   ...
   {
      DWORD bytesRead{};
      const auto res = ReadFile(reinterpret_cast<HANDLE>(m_file), buffer.data(), static_cast<DWORD>(buffer.size()), &bytesRead,
                                nullptr);
      if (not res) {
         return std::unexpected(Status::BrokenPipe);
      }
   ...
   {
      DWORD bytesWritten{};
      const auto res = WriteFile(reinterpret_cast<HANDLE>(m_file), buffer.data(), static_cast<DWORD>(buffer.size()), &bytesWritten,
                                 nullptr);
      if (not res) {
         return std::unexpected(Status::BrokenPipe);
      }
