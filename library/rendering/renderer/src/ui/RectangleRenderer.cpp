...
RectangleRenderer::RectangleRenderer(graphics_api::Device& device, ui_core::Viewport& viewport) :
    m_draw_calls{GAPI_CHECK(device.create_buffer(graphics_api::BufferUsage::TransferDst | graphics_api::BufferUsage::Indirect |
                                                    graphics_api::BufferUsage::StorageBuffer,
                                                 sizeof(DrawCall) * g_draw_call_buffer_size))},
...
void RectangleRenderer::on_updated_rectangle(const RectId rect_id, const ui_core::Rectangle& rect)
{
   ...
   m_frame_updates.add_or_update(rect_id, to_primitive(rect));
}

void RectangleRenderer::on_removed_rectangle(const RectId rect_id)
{
   std::unique_lock lk{m_rect_update_mtx};
   m_frame_updates.remove(rect_id);
}
...
void RectangleRenderer::prepare_frame(render_core::JobGraph& graph, const u32 frame_index)
{
   ...
   count.cast<u32>() = m_frame_updates.top_index();
}
...
void RectangleRenderer::build_render_ui(render_core::BuildContext& ctx)
{
   ...
   ctx.bind_storage_buffer(1, &m_draw_calls);

   ctx.bind_fragment_shader("shader/ui/rectangle_render.fshader"_rc);

   ctx.draw_indirect_with_count(&m_draw_calls, "user_interface.rectangle.count"_external, g_draw_call_buffer_size, sizeof(DrawCall));
}
