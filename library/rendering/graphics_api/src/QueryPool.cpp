...
void QueryPool::get_result(std::span<float> out, const u32 first) const
{
   ...
   vkGetQueryPoolResults(m_queryPool.parent(), *m_queryPool, first, static_cast<u32>(timestamps.size()), sizeof(u64) * timestamps.size(),
...
float QueryPool::get_difference(const u32 begin, const u32 end) const
{
   ...
   vkGetQueryPoolResults(m_queryPool.parent(), *m_queryPool, begin, static_cast<u32>(timestamps.size()), sizeof(u64) * timestamps.size(),
                         timestamps.data(), sizeof(u64), VK_QUERY_RESULT_64_BIT | VK_QUERY_RESULT_WAIT_BIT);
