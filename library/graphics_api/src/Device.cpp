...
bool is_surface_format_supported(const VkPhysicalDevice physicalDevice, const vulkan::SurfaceKHR& surface, const ColorFormat& colorFormat,
...
Result<Swapchain> Device::create_swapchain(ColorFormat colorFormat, ColorSpace colorSpace, const Resolution& resolution,
...
#endif
                                   std::move(vulkan_surface), std::move(device), *pickedDevice, std::move(queueFamilyInfos));
}
